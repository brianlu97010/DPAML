...
PATH_TO_CKPT = 'c:\\EX8\\EX8-2\\posenet_mobilenet_v1_100_257x257_multi_kpt_stripped.tflite'
...
name='EX8-2\\DrawModel_64x64.tflite'
...
while True:
    ...
    x_sum = 0
    y_sum = 0
    for point in positions_list:
        x_sum += point[0]
        y_sum += point[1]
        
    # 計算座標的平均值
    x_mean = int(x_sum / len(positions_list))
    y_mean = int(y_sum / len(positions_list))
    ...
    positions_list_norm = []
    for point in positions_list:
        new_x = point[0] - x_norm
        new_y = point[1] - y_norm
        positions_list_norm.append((new_x, new_y))
    draw_pic = Image.new('RGB', (64, 64), (255, 255, 255))
    draw = ImageDraw.Draw(draw_pic)
    
    # 畫出火柴人
    nose = positions_list_norm[0]
    left_eye = positions_list_norm[1]
    right_eye = positions_list_norm[2]
    left_ear = positions_list_norm[3]
    right_ear = positions_list_norm[4]
    left_shoulder = positions_list_norm[5]
    right_shoulder = positions_list_norm[6]
    left_elbow = positions_list_norm[7]
    right_elbow = positions_list_norm[8]
    left_wrist = positions_list_norm[9]
    right_wrist = positions_list_norm[10]
    left_hip = positions_list_norm[11]
    right_hip = positions_list_norm[12]
    left_knee = positions_list_norm[13]
    right_knee = positions_list_norm[14]
    left_ankle = positions_list_norm[15]
    right_ankle = positions_list_norm[16]
    
    draw.line([left_ear, left_eye, nose, right_eye, right_ear], fill = (0,0,255), width = 4)
    draw.line([nose, right_shoulder, left_shoulder], fill=(0,0,255), width = 4)
